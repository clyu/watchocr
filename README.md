...
- **Directory monitoring**: pick a folder (via SAF) and a foreground service polls it every 5 seconds for newly added image files. On first run it baselines existing files without processing them, so only files added afterwards are OCR'd.
...
1. In **Settings**, choose a directory to watch, enter your Gemini API key, and optionally change the model (defaults to `gemini-3.1-flash-lite`).
2. Once both a directory and API key are set, `DirectoryMonitorService` starts as a foreground service and polls the directory for new `.jpg`/`.jpeg`/`.png` (or `image/*`) files.
...
│   ├── OcrRecordDao.kt
│   ├── MonitoredFile.kt          # Tracks already-seen files in the watched dir
│   ├── MonitoredFileDao.kt
...
├── service/
│   └── DirectoryMonitorService.kt # Foreground service that polls the watched dir
└── ui/
    ├── HistoryScreen.kt
    ├── SettingsScreen.kt
    └── theme/
...
- Storage Access Framework (`DocumentFile`) for directory monitoring without broad storage permissions
